...
#define LLHLS_PARTIAL_PTS 1000000
...
static int parse_variant_stream_mapstring(AVFormatContext *s)
{
    ...
    hls->var_streams = av_mallocz(sizeof(*hls->var_streams) * nb_varstreams * 2);
    if (!hls->var_streams)
        return AVERROR(ENOMEM);
...
static int update_variant_stream_info(AVFormatContext *s)
{
    ...
    {
        //By default, a single variant stream with all the codec streams and a variant stream for llhls-partial are created
        hls->var_streams = av_mallocz(sizeof(*hls->var_streams) * 2);
        if (!hls->var_streams)
            return AVERROR(ENOMEM);
        hls->nb_varstreams = 1;

        for (j = 0; j < hls->nb_varstreams * 2; j++)
        {
            hls->var_streams[j].var_stream_idx = j;
            hls->var_streams[j].nb_streams = s->nb_streams;
            hls->var_streams[j].streams = av_mallocz(sizeof(AVStream *) * hls->var_streams[j].nb_streams);
            if (!hls->var_streams[j].streams)
                return AVERROR(ENOMEM);

            //by default, the first available ccgroup is mapped to the variant stream
            if (hls->nb_ccstreams)
            {
                hls->var_streams[j].ccgroup = hls->cc_streams[0].ccgroup;
            }
        }
        for (i = 0; i < s->nb_streams; i++)
        {
            for (j = 0; j < hls->nb_varstreams * 2; j++)
            {
                hls->var_streams[j].streams[i] = s->streams[i];
            }
        }
    }
    return 0;
...
static int hls_write_header(AVFormatContext *s)
{
    ...
    for (i = 0; i < hls->nb_varstreams * 2; i++)
    {
        vs = &hls->var_streams[i];
...
{
    HLSContext *hls = s->priv_data;
    AVFormatContext *segment_oc = NULL;
    AVFormatContext *partial_oc = NULL;
    ...
    {
        segment_vs = &hls->var_streams[i];
        partial_vs = &hls->var_streams[i + 1];
        for (j = 0; j < segment_vs->nb_streams; j++)
        {
            if (segment_vs->streams[j] == st)
            {
                if (st->codecpar->codec_type == AVMEDIA_TYPE_SUBTITLE)
                {
                    segment_oc = segment_vs->vtt_avf;
                    partial_oc = partial_vs->vtt_avf;
                    stream_index = 0;
                }
                else
                {
                    segment_oc = segment_vs->avf;
                    partial_oc = partial_vs->avf;
                    stream_index = j;
                }
                break;
            }
        }
        if (segment_oc != NULL && partial_oc != NULL)
            break;
    }
    if (segment_oc == NULL || partial_oc == NULL)
    {
    ...
    if (segment_vs->sequence - segment_vs->nb_entries > hls->start_sequence && hls->init_time > 0)
    {
        ...
        int64_t init_list_dur = hls->init_time * segment_vs->nb_entries * AV_TIME_BASE;
        int64_t after_init_list_dur = (segment_vs->sequence - hls->start_sequence - segment_vs->nb_entries) * (hls->time * AV_TIME_BASE);
        hls->recording_time = hls->time * AV_TIME_BASE;
        end_pts = init_list_dur + after_init_list_dur;
    }

    if (segment_vs->start_pts == AV_NOPTS_VALUE)
    {
    ...
    if (segment_vs->has_video)
    {
        ...
        is_ref_pkt = (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) && (pkt->stream_index == segment_vs->reference_stream_index);
    }
    ...
    if (is_ref_pkt)
    {
        ...
        if (segment_vs->new_start)
        {
            ...
            segment_vs->duration = (double)(pkt->pts - segment_vs->end_pts) * st->time_base.num / st->time_base.den;
            segment_vs->dpp = (double)(pkt->duration) * st->time_base.num / st->time_base.den;
        }
        else
        {
            if (pkt->duration)
            {
                segment_vs->duration += (double)(pkt->duration) * st->time_base.num / st->time_base.den;
            }
            else
            {
    ...
    if (segment_oc->pb)
    {
        ret = ff_write_chained(segment_oc, stream_index, pkt, s, 0);
        segment_vs->video_keyframe_size += pkt->size;
        if ((st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) && (pkt->flags & AV_PKT_FLAG_KEY))
        {
            segment_vs->video_keyframe_size = avio_tell(segment_oc->pb);
        }
        else
        {
            segment_vs->video_keyframe_pos = avio_tell(segment_vs->out);
        }
        if (hls->ignore_io_errors)
            ret = 0;
    }

    partial_vs->packets_written++;
    if (partial_oc->pb)
    {
        ret = ff_write_chained(partial_oc, stream_index, pkt, s, 0);
        partial_vs->video_keyframe_size += pkt->size;
        if ((st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) && (pkt->flags & AV_PKT_FLAG_KEY))
        {
            partial_vs->video_keyframe_size = avio_tell(partial_oc->pb);
        }
        else
        {
            partial_vs->video_keyframe_pos = avio_tell(partial_vs->out);
        }
        if (hls->ignore_io_errors)
            ret = 0;
    }
...
static int hls_init(AVFormatContext *s)
{
    ...
    for (i = 0; i < hls->nb_varstreams * 2; i++)
    {
        ...
        ret = format_name(s->url, &vs->m3u8_name, i / 2, vs->varname);
        if (ret < 0)
            return ret;
        ...
        if (hls->segment_filename)
        {
            ret = format_name(hls->segment_filename, &vs->basename, i / 2, vs->varname);
            if (ret < 0)
                return ret;
        }
        else
        ...
        if (hls->segment_type == SEGMENT_TYPE_FMP4)
        {
            ...
            else
            {
                ...
                if (!vs->fmp4_init_filename)
                    return AVERROR(ENOMEM);
                av_strlcpy(vs->fmp4_init_filename, hls->fmp4_init_filename, fmp4_init_filename_len);
                if (hls->nb_varstreams > 1)
                {
                    if (av_stristr(vs->fmp4_init_filename, "%v"))
                    {
                        ...
                        ret = format_name(hls->fmp4_init_filename,
                                          &vs->fmp4_init_filename, i / 2, vs->varname);
                    }
                    else
                    {
                ...
                if (p)
                {
                    char tmp = *(++p);
                    *p = '\0';
                    vs->base_output_dirname = av_asprintf("%s%s", vs->m3u8_name, vs->fmp4_init_filename);
                    *p = tmp;
                }
                else
                {
