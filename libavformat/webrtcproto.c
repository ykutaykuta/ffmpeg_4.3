...
{
    av_log(NULL, AV_LOG_INFO, "ykuta %s begin size: %d\n", __func__, size);
    ...
    while (true)
    {
        if (sender->isHasPeer && sender->state == RTC_CONNECTED)
        {
            if (sender->isVideoConnected)
            {
                ...
                {
                    uint32_t timestamp;
                    rtcGetCurrentTrackTimestamp(sender->audio, &timestamp);
                    av_log(NULL, AV_LOG_INFO, "ykuta track timestamp: %d\n", timestamp);
                    ret = max_size;
    ...
    av_log(NULL, AV_LOG_INFO, "ykuta %s end ret = %d\n", __func__, ret);
    return ret;
}
